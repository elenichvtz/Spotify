...
public class PublisherNode implements Publisher,Serializable{
    ...
    public int getBrokerPort1(){return this.BrokerPort1;}

    public int getBrokerPort2(){return this.BrokerPort2;}

    public int getBrokerPort3(){return this.BrokerPort3;}

    public void setSocketPort(int port) {
        try {
            this.requestSocket = new Socket(this.ip, port);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
        ...
        publishers.parallelStream().forEach((publisher) -> {
            ...
            //for(int i=0; i<3; i++) {
                try {
                    ...
                    /*if(i==1) {
                        publisher.setPort(publisher.getBrokerPort1());
                        publisher.setSocketPort(publisher.getBrokerPort1());
                    }
                    if(i==2) {
                        publisher.setPort(publisher.getBrokerPort2());
                        publisher.setSocketPort(publisher.getBrokerPort2());
                    }
                    if(i==3) {
                        publisher.setPort(publisher.getBrokerPort3());
                        publisher.setSocketPort(publisher.getBrokerPort3());
                    }

                    System.out.println(i+" "+publisher.getPublisherPort());
                    System.out.println(publisher.getSocket().getPort());*/
