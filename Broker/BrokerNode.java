...
public class BrokerNode implements Broker{
    ...
    @Override
    public void pull(ArtistName artist) throws NoSuchAlgorithmException {
        ...
        //edw o broker psaxnei sto map tou kathe publisher gia tous kallitexnes
        for(int i=0; i<registeredPublishers.size();i++) {

            for (Map.Entry<String, ArrayList<String>> entry : registeredPublishers.get(i).getArtistMap().entrySet()) {

                if (artist.artistName.equals(entry.getKey())) {       

                    for (int j = 0; j < getBrokers().size(); j++) {
                        if (entry.getKey().equals(registeredPublishers.get(i).hashTopic(artist).getBrokers().get(j))) {
                            try {
                                this.out.writeObject(entry.getValue().get(i));
                                this.out.flush();
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
