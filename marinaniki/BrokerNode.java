...
public class BrokerNode extends Thread implements Broker,Serializable {
    ...
    public void notifyPublisher(String name) {
        ...
        try {
            ...
            if(name.charAt(0) >= 'A' && name.charAt(0) <= 'M') { //we find the right publisher
                this.out.writeUTF(registeredPublishers.get(0).getPublisherIP());
                this.out.writeInt(registeredPublishers.get(0).getPublisherPort());
                this.out.writeObject(name); //successfully sends artistName to BrokerNode
                this.out.flush();
            } else {
                this.out.writeUTF(registeredPublishers.get(1).getPublisherIP());
                this.out.writeInt(registeredPublishers.get(1).getPublisherPort());
                this.out.writeObject(name); //successfully sends artistName to BrokerNode
                this.out.flush();
            }
            ...
            /*try {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        System.out.println("consumer ip is " + consumerip);
