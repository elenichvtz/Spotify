...
public class PublisherNode implements Publisher,Serializable{
    ...
    public void push(ArtistName artist,Value val) { //stin main tou publisher
            ...
            int i=0;
            boolean flag = false;
            while(i< artistMap.size()) {
                if (artist.artistName.equals(artistMap.get(i))){
                    flag = true;
                    break;
                }
                i++;
    ...
    @Override
    public void notifyFailure(){ // nomizw den 8elei BrokerNode orisma
        //TODO: write code

        try {

            out.writeUTF("I don't have the song please select another one");
            out.flush();
        } catch (IOException e) {
            e.printStackTrace();
        }
